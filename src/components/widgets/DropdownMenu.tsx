    ...
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button
          ...
          aria-expanded="true"
          aria-haspopup="true"
        >İndir {data.title}<svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
        </Menu.Button>
      </div>
      ...
      >
        <Menu.Items id="editions-menu-list" className="dropdown-menu min-w-max absolute bg-white dark:bg-gray-800 text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 m-0 bg-clip-padding border-none">
          <Menu.Item as="a" key={data.direct_url} href={data.direct_url} className="btn-dropdown-item block" onClick={async () => await handleDirectButton(data.title)}>Direkt İndir</Menu.Item>
          {data.srcforge_url && <Menu.Item as="a" key={data.srcforge_url} href={data.srcforge_url} className="btn-dropdown-item block">Discord</Menu.Item>}
        </Menu.Items>
      </Transition>
    </Menu>
  )
